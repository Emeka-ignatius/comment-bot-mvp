import { mysqlTable, mysqlSchema, AnyMySqlColumn, foreignKey, int, mysqlEnum, varchar, text, timestamp, index } from "drizzle-orm/mysql-core"
...
export const jobs = mysqlTable("jobs", {
	...
	commentTemplateId: int().notNull().references(() => commentTemplates.id, { onDelete: "cascade" } ),
...
export const logs = mysqlTable("logs", {
	...
	errorDetails: text(),
	createdAt: timestamp({ mode: 'string' }).default('CURRENT_TIMESTAMP').notNull(),
	accountId: int().references(() => accounts.id, { onDelete: "set null" } ),
	videoId: int().references(() => videos.id, { onDelete: "set null" } ),
	action: mysqlEnum(['manual_comment','ai_comment','job_comment']).default('job_comment').notNull(),
	metadata: text(),
});
