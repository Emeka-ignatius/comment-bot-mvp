...
function getMapsConfig(): MapsConfig {
  // Google Maps API requires a Google Maps API key
  // Set GOOGLE_MAPS_API_KEY environment variable to use this feature
  const apiKey = process.env.GOOGLE_MAPS_API_KEY;
  
  if (!apiKey) {
    throw new Error(
      "Google Maps API key is missing: set GOOGLE_MAPS_API_KEY environment variable"
    );
  }
